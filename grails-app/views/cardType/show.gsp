	...
	<body>
		...
		<div id="show-cardType" class="content scaffold-show" role="main">
			...
			<ol class="property-list cardType">
				...
				<g:if test="${cardTypeInstance?.bank}">
				<li class="fieldcontain">
					<span id="bank-label" class="property-label"><g:message code="cardType.bank.label" default="Bank" /></span>
					
						<span class="property-value" aria-labelledby="bank-label"><g:link controller="bank" action="show" id="${cardTypeInstance?.bank?.id}">${cardTypeInstance?.bank?.encodeAsHTML()}</g:link></span>
